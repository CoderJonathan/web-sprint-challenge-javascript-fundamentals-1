...
// Because the nestedFunction is inside the same scope as the internal variable. They both are inside the myFunction function.
...
const zooAnimals = [
  ...
  */
  function lowerCaseNames(list){
    return list.map(item => item.animal_name.toLowerCase())
  }
  ...
  */
    function lowPopulationAnimals(amount){

      const lowPopulationAnimals = amount.filter(item => item.population < 5)
 ...
 function USApop(total){
  const totalPop = total.reduce((accumulator,item) => accumulator + item.population, 0);
  return totalPop;
}
